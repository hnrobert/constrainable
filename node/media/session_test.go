...
func TestCheckLimitsBitrateVideoSemantics(t *testing.T) {
	...
	if r := checkLimits(&Session{Width: 1280, Height: 720, Fps: 30, VideoBitrateKbps: 1000}, lim); len(r) != 0 {
		t.Fatalf("at-cap stream flagged: %v", r)
	}
	// inside the 10% headroom (overhead/ABR) → clean
	if r := checkLimits(&Session{Width: 1280, Height: 720, Fps: 30, VideoBitrateKbps: 1090}, lim); len(r) != 0 {
		t.Fatalf("within-headroom stream flagged: %v", r)
	}
	// clearly over → flagged
	if r := checkLimits(&Session{Width: 1280, Height: 720, Fps: 30, VideoBitrateKbps: 1200}, lim); len(r) != 1 || r[0] != "bitrate exceeds limit" {
		t.Fatalf("over-cap stream: %v", r)
	}
}

// SetDeclaredAudioBitrateKbps clamps: a forged huge audiodatarate must not zero the
...
func TestSetDeclaredAudioClamped(t *testing.T) {
	...
	m.SetDeclaredAudioBitrateKbps("s", 100000)
	if got := m.sessions["s"].declaredAudioBitrateKbps; got != 320 {
		t.Fatalf("forged audio declaration clamped to 320, got %d", got)
	}
	m.SetDeclaredAudioBitrateKbps("s", -5)
	if got := m.sessions["s"].declaredAudioBitrateKbps; got != 0 {
		t.Fatalf("negative audio declaration clamped to 0, got %d", got)
	}
	m.SetDeclaredAudioBitrateKbps("s", 160)
	if got := m.sessions["s"].declaredAudioBitrateKbps; got != 160 {
		t.Fatalf("sane audio declaration kept, got %d", got)
	}

	// unknown stream → no-op, no panic
	m.SetDeclaredAudioBitrateKbps("missing", 128)
}
